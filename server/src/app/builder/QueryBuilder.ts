...
class QueryBuilder<T> {
  ...
  yearFilter(dateField: string) {
      // dateField check
      if (!dateField) {
        return this;
      }

      const fromDate = this.query.startYear as string;
      const toDate = this.query.endYear as string;

      if (!fromDate || !toDate) {
        return this;
      }

      // If date string, extract year
      ...
      const fromYear = extractYear(fromDate);
      const toYear = extractYear(toDate);

      if (isNaN(fromYear) || isNaN(toYear)) {
        return this;
      }

      // year থেকে date range বানানো
      const startDate = new Date(Date.UTC(fromYear, 0, 1, 0, 0, 0, 0)); // Jan 1, fromYear
      const endDate = new Date(Date.UTC(toYear, 11, 31, 23, 59, 59, 999)); // Dec 31, toYear
      ...
      delete this.query.startYear;
      delete this.query.endYear;
      this.modelQuery = this.modelQuery.find(filterCondition as FilterQuery<T>);
      return this;
  }
